...
def get_project_path(toolbox_path, name):
    # creates a default path for a cbm project databases in the toolbox
    # installation dir
    return os.path.join(toolbox_path, "Projects", name, "{}.mdb".format(name))


def get_results_path(project_path):
    # creates a default path for a cbm results database in the toolbox
    # installation dir
...
def import_cbm3_project(name, cases, age_interval, num_age_classes, nsteps,
                        cbm_exe_path, toolbox_path, archive_index_db_path,
    ...
    for c in cases:
        ...
        sit_config.add_inventory(
            classifier_set=cset, area=c["area"], age=c["age"],
