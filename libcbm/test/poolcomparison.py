...
def get_cbm3_dom_pools():
    return ['Aboveground Very Fast DOM', 'Belowground Very Fast DOM',
...
def get_libcbm_biomass_pools():
     return ['SoftwoodMerch', 'SoftwoodFoliage', 'SoftwoodOther',
...
def get_comparison(name):
    if name=="all":
      return get_pool_mapping(get_cbm3_pools(),
                              get_libcbm_pools())
    elif name=="biomass":
        return get_pool_mapping(get_cbm3_biomass_pools(),
                                get_libcbm_biomass_pools())
    else:
        raise ValueError("unknown comparison name, expected 'all' or 'biomass'")


def get_pool_mapping(cbm_pools, libcbm_pools):
    """returns a mapping between libcbm poolnames and cbm-cfs3 poolnames assumes the pools are ordered"""
