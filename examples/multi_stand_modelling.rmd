...
spinup <- function(parameter_factory, inventory, classifiers, merch_volumes ){
...
```{r}
step <- function(parameter_factory, cbm_vars){
  with(cbm_factory$initialize_cbm(
    # note we pass the customized parameter factory here
    cbm_parameters_factory=parameter_factory
  ) %as% cbm, {
...
Create a `CBMOutput`object to store the time step output. 
...
```{r}
cbm_vars = spinup(parameter_factory, inventory, classifiers, merch_volumes)
...
for (i in 1:50){
  ...
  step(parameter_factory, cbm_vars)
